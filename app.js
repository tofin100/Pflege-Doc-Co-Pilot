// app.js – Pflege SIS Co-Pilot
// Version: Pflege / SIS (auf Basis Physio Doc Co-Pilot)

const STORAGE_KEY = "pflegeSISPilot_v1";

let state = {
  patients: [],          // Klienten/Bewohner
  selectedPatientId: null,
  selectedSessionId: null
};

// ---------------- Optionen: Pflegeanlass & Maßnahmen ----------------

const COMPLAINT_OPTIONS = [
  { id: "pain",          label: "Schmerz / Unwohlsein" },
  { id: "disorientation",label: "Desorientierung / Verwirrtheit" },
  { id: "fall_risk",     label: "Sturzrisiko / Sturzereignis" },
  { id: "breath",        label: "Atemnot / respiratorische Probleme" },
  { id: "wound",         label: "Wunde / Hautproblem" },
  { id: "nutrition",     label: "Ernährung / Flüssigkeitsaufnahme" },
  { id: "excretion",     label: "Ausscheidung / Inkontinenz" },
  { id: "mobility",      label: "eingeschränkte Mobilität" }
];

const MEASURE_OPTIONS = [
  { id: "basic_care",    label: "Grundpflege (Körperpflege, An-/Auskleiden)" },
  { id: "treatment_care",label: "Behandlungspflege (z. B. Wunde, Injektion)" },
  { id: "medication",    label: "Medikamentengabe / Kontrolle" },
  { id: "mobilisation",  label: "Mobilisation / Lagerung" },
  { id: "nutrition_support", label: "Unterstützung Essen/Trinken" },
  { id: "excretion_support", label: "Unterstützung bei Ausscheidung" },
  { id: "counseling",    label: "Beratung / Anleitung Klient/Angehörige" },
  { id: "documentation", label: "Organisation / Dokumentation / Abstimmung" }
];
...
// strukturierte Textfelder (SIS-Bereiche)
const speechAnamneseEl     = document.getElementById("speech-anamnese");      // Biografie & Gewohnheiten
const speechBefundEl       = document.getElementById("speech-befund");        // TF 1
const speechDiagnoseEl     = document.getElementById("speech-diagnose");      // TF 2
const speechTherapieplanEl = document.getElementById("speech-therapieplan");  // TF 3
const speechVerlaufEl      = document.getElementById("speech-verlauf");       // TF 4
const speechEpikriseEl     = document.getElementById("speech-epikrise");      // TF 5
const speechNotesEl        = document.getElementById("speech-notes");         // TF 6
...
// Interpretation: 0 = keine Belastung, 100 = hohe Belastung
function calculateScore({ pain, func, complaintsCount }) {
  const painNorm = (pain / 10) * 100; // subjektive Belastung
  const funcNorm = (func / 10) * 100; // Einschränkung Alltag
  const compNorm = (Math.min(complaintsCount, 5) / 5) * 100; // Anzahl Themen
...
function scoreCategoryFromValue(score) {
  if (score < 34) return { text: "geringe Pflegebelastung",  color: "#9ae6b4" };
  if (score < 67) return { text: "mittlere Pflegebelastung", color: "#faf089" };
  return { text: "hohe Pflegebelastung",                      color: "#feb2b2" };
}

// Generiert eine strukturierte SIS-/Pflege-Doku aus den Feldern der Sitzung
function generateNoteForSession(patient, session) {
  const typeLabel =
    session.type === "initial"
      ? "Erstaufnahme / SIS-Erstfassung"
      : "Fortschreibung / Folgeeintrag";
  ...
  const nameLabel = patient.name || "Unbenannter Klient/Bewohner";
  ...
  const header =
    ...
    `Klient/Bewohner: ${nameLabel}\n` +
    `Diagnose (ICD-10, soweit bekannt): ${diagLabel}\n` +
    `Belastungs-Score: ${score}/100 (${scoreCat.text}) – ` +
    `subjektive Belastung ${pain}/10, Einschränkung im Alltag ${func}/10.\n`;

  // ------- Pflegeanlass / Hauptthema -------
  let anlass = "Pflegeanlass / Hauptthemen:\n";
  if (complaintLabels.length) {
    anlass += `• Aktuell im Vordergrund: ${complaintLabels.join(", ")}.\n`;
  } else {
    anlass += "• Allgemeine pflegerische Versorgung ohne spezifische Schwerpunktangabe.\n";
  }

  // ------- Biografie & Gewohnheiten -------
  let biografieBlock = "\nBiografie & Gewohnheiten (SIS – Biografie/Alltag):\n";
  if (session.anamnese && session.anamnese.trim()) {
    biografieBlock += session.anamnese.trim() + "\n";
  } else {
    biografieBlock +=
      "• Kurzbiografie, bisherige Lebenssituation, Gewohnheiten und Ressourcen wurden im Aufnahmegespräch erhoben.\n";
  }

  // ------- TF 1–6 -------
  let tf1 = "\nTF 1 – Kognition & Kommunikation:\n";
  tf1 += session.befund && session.befund.trim()
    ? session.befund.trim() + "\n"
    : "• Orientierung, Erinnerung und Kommunikationsfähigkeit orientierend eingeschätzt; besondere Auffälligkeiten werden bei Bedarf ergänzt.\n";

  let tf2 = "\nTF 2 – Mobilität & Bewegung:\n";
  tf2 += session.diagnose && session.diagnose.trim()
    ? session.diagnose.trim() + "\n"
    : "• Mobilität, Transfers und Sturzrisiko wurden beobachtet; vorhandene Ressourcen werden in der Planung berücksichtigt.\n";

  let tf3 = "\nTF 3 – Krankheitsbezogene Anforderungen & Belastungen:\n";
  tf3 += session.therapieplan && session.therapieplan.trim()
    ? session.therapieplan.trim() + "\n"
    : "• Vorliegende Erkrankungen, Therapien, Medikation und Monitoring-Aufgaben sind bekannt; besondere Anforderungen werden im Pflegeplan abgebildet.\n";

  let tf4 = "\nTF 4 – Selbstversorgung:\n";
  tf4 += session.verlauf && session.verlauf.trim()
    ? session.verlauf.trim() + "\n"
    : "• Fähigkeit zur Selbstversorgung in den Bereichen Körperpflege, Ernährung, Ausscheidung und Umgang mit Hilfsmitteln wurde eingeschätzt.\n";

  let tf5 = "\nTF 5 – Leben in sozialen Beziehungen:\n";
  tf5 += session.epikrise && session.epikrise.trim()
    ? session.epikrise.trim() + "\n"
    : "• Familiäre Situation, soziale Kontakte und Teilhabe sind bekannt; Wünsche und Bedürfnisse werden berücksichtigt.\n";

  let tf6 = "\nTF 6 – Haushaltsführung / Sonstiges:\n";
  tf6 += session.speechNotes && session.speechNotes.trim()
    ? session.speechNotes.trim() + "\n"
    : "• Unterstützung in der Haushaltsführung (z. B. Einkaufen, Kochen, Haushaltsorganisation) wird je nach Bedarf sichergestellt.\n";

  // ------- Durchgeführte Maßnahmen / Leistungen -------
  let leistungen = "\nHeute durchgeführte Leistungen / Maßnahmen:\n";
  if (measureLabels.length) {
    leistungen += `• ${measureLabels.join(", ")}.\n`;
  } else {
    leistungen += "• Allgemeine pflegerische Unterstützung entsprechend Pflegegrad und aktueller Situation.\n";
  }

  // ------- Pflegerische Einschätzung & Ziele -------
  let einschätzung = "\nPflegerische Einschätzung & Ziele:\n";
  einschätzung +=
    `• Gesamtbelastung aktuell bewertet mit ${score}/100 (${scoreCat.text}).\n` +
    "• Ziele u. a.: Erhalt und Förderung von Ressourcen, Reduktion von Risiken (z. B. Sturz, Dekubitus), " +
    "Stabilisierung des Allgemeinzustands und Unterstützung der Selbstständigkeit soweit möglich.\n";

  // Gesamtdoku zusammensetzen
  return (
    header +
    "\n" +
    anlass +
...
function createNewSession(patient, type = "initial") {
  const session = {
    ...
    pain: 5,
    function: 5,
    // strukturierte Bereiche (SIS)
    anamnese: "",     // Biografie & Gewohnheiten
    befund: "",       // TF 1
    diagnose: "",     // TF 2
    therapieplan: "", // TF 3
    verlauf: "",      // TF 4
...
function renderPatients() {
  ...
  if (!state.patients.length) {
    ...
    li.textContent = "Noch keine Klienten – lege links einen neuen an.";
    li.className = "meta";
  ...
  state.patients.forEach((p) => {
    ...
    nameSpan.textContent = p.name || "Unbenannter Klient/Bewohner";
...
function renderPatientDetail() {
  ...
  patientTitleEl.textContent = patient.name || "Unbenannter Klient/Bewohner";
  const meta = [];
...
function renderSessions(patient) {
  ...
  if (!patient.sessions || !patient.sessions.length) {
    const li = document.createElement("li");
    li.textContent = "Noch keine Pflegeeinträge";
    li.className = "meta";
    li.style.cursor = "default";
  ...
  sorted.forEach((s) => {
    ...
    const typeLabel =
      s.type === "initial" ? "Erstaufnahme / SIS" : "Fortschreibung";
    ...
    if (typeof s.score === "number") {
      meta.textContent = `Belastungs-Score ${s.score}`;
    } else {
      meta.textContent = "";
    }
...
function labelForTarget(targetId) {
  switch (targetId) {
    case "speech-anamnese":      return "Biografie & Gewohnheiten";
    case "speech-befund":        return "TF 1 – Kognition & Kommunikation";
    case "speech-diagnose":      return "TF 2 – Mobilität & Bewegung";
    case "speech-therapieplan":  return "TF 3 – krankheitsbez. Anforderungen";
    case "speech-verlauf":       return "TF 4 – Selbstversorgung";
    case "speech-epikrise":      return "TF 5 – soziale Beziehungen";
    case "speech-notes":         return "TF 6 – Haushaltsführung / Sonstiges";
...
function setupEventListeners() {
  newPatientForm.addEventListener("submit", (e) => {
    ...
    if (!name) {
      alert("Bitte einen Namen für den Klienten/Bewohner eingeben.");
      return;
    }
  ...
  copyNoteBtn.addEventListener("click", async () => {
    ...
    if (!text.trim()) {
      alert("Keine Dokumentation zum Kopieren vorhanden.");
      return;
    }
    try {
  ...
  saveSessionBtn.addEventListener("click", () => {
    saveState();
    alert("Pflegeeintrag gespeichert (lokal im Browser).");
  });
  ...
  deleteSessionBtn.addEventListener("click", () => {
    ...
    const session = getSelectedSession(patient);
    if (!patient || !session) return;
    if (!confirm("Pflegeeintrag wirklich löschen?")) return;
